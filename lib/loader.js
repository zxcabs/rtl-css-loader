...
var cssDiff = require('@romainberger/css-diff');
...
module.exports = function(content, map) {
    ...
    var dirTagName = query.dirTagName || 'html';

    if (query.diff) {
      var replaceReg = new RegExp(dirTagName + '(\\s{0,}[\[\{]?)', 'g');
      rtlcssAsString = cssDiff(cssAsString, rtlcssAsString);
      rtlcssAsString = scopeCss(rtlcssAsString.slice(1).replace(replaceReg, '$1'), dirTagName + '[dir=rtl]');
    }
    ...
    if (query.diff) {
      cssString = cssAsString.slice(0, cssAsString.length - 1) + rtlcssAsString;
    } else {
      cssString = "typeof document !== 'undefined' && document.getElementsByTagName('" + dirTagName + "')[0].getAttribute('dir') === 'rtl' ? " + rtlcssAsString + " : " + cssAsString;
    }
		...
		if(query.sourceMap && result.map) {
			...
			map = JSON.stringify(map);
			moduleJs = "exports.push([module.id, " + cssString + ", \"\", " + map + "]);";
		} else {
			moduleJs = "exports.push([module.id, " + cssString + ", \"\"]);";
		}
